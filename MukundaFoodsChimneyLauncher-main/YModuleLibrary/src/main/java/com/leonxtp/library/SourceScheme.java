...
//            uriPrefixLen = uriPrefix.length();
//            return uri.substring(uriPrefixLen);
//
//        }
//        return null;
//    }
    public String crop(String uri) {
        if (!belongsTo(uri)) {
            throw new IllegalArgumentException(
                    String.format("URI [%1$s] doesn't have expected scheme [%2$s]", uri, scheme)
            );
        }

        if (uriPrefix != null && !uriPrefix.isEmpty()) {
            int uriPrefixLen = uriPrefix.length();
            return uri.substring(uriPrefixLen);
        } else {
            throw new IllegalStateException("uriPrefix is null or empty for scheme: " + scheme);
        }
    }
