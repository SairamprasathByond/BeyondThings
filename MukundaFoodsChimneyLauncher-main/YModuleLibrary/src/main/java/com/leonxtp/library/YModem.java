...
public class YModem implements FileStreamThread.DataRaderListener {
    ...
    public static  int funcack_expected()
    {
        return ack_expected;
    }
    /**
    ...
    public void onReceiveData(byte[] respData) {
        ...
        if (respData != null && respData.length > 0) {
            L.f("edison YModem received " + respData.length + " bytes.");
            switch (CURR_STEP) {
                case STEP_HELLO:
                    ack_expected=0;
                ...
                case STEP_FILE_NAME:
                    ack_expected=0;
                    handleFileName(respData,respData.length);
                  //  Display("handling File name...");
                    break;
                case STEP_FILE_BODY:
                    ack_expected=1;
                    handleFileBody(respData,respData.length);
                  //  Display("Connecting...");
                    break;
                case STEP_EOT:
                    ack_expected=1;
            ...
            L.f("edison The terminal do responsed something, but received nothing??");
        }
    }
    ...
    private void sendPackageData(byte[] packageData) {
        if (listener != null && packageData != null) {
            ...
            timerHelper.startTimer(timeoutListener, 2000);//PACKAGE_TIME_OUT);
    ...
    private void handleFileName(byte[] value, int len) {
        int i;
       ...
       if (sub.length() == 2 && value[0] == ACK && value[1] == ST_C) {//Receive 'ACK C' for file name
         //   if ( value[0] == ACK && value[1] == ST_C) {//Receive 'ACK C' for file name
                L.f("edison Received 'ACK C'");
            packageErrorTimes = 0;
            startSendFileData();
    ...
    private void handleFileBody(byte[] value, int len) {
        //int i;

        if ( value[0] == ACK) {//Receive ACK for file data
            L.f("edison Received  file 'ACK'");
            packageErrorTimes = 0;
    ...
    private void handleEnd(byte[] character) {
        if (character[0] == ACK) {//The last ACK represents that the transmission has been finished, but we should validate the file
            ...
            L.f("edison Received 'MD5_OK'");
            stop();
            if (listener != null) {
                listener.onSuccess();
            }
        } else if ((new String(character)).equals(MD5_ERR)) {//Oops...Transmission Failed...
            L.f("edison Received 'MD5_ERR'");
            stop();
            if (listener != null) {
                listener.onFailed("edison MD5 check failed!!!");
            }
        } else {
    ...
    private void handlePackageFail(String reason) {
        packageErrorTimes++;
        L.f("edison Fail:" + reason + " for " + packageErrorTimes + " times");
