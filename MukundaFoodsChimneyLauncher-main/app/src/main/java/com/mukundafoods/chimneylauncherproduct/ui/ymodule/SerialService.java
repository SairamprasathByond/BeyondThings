...
public class SerialService extends Service implements SerialListener {
    ...
    private enum QueueType {Connect, ConnectError, Read, IoError, Write}
        ...
        for (QueueItem item : queue1) {
            switch (item.type) {
                ...
                case IoError:
                    ...
                    break;
                case Write:
                    listener.onSerialWrite(item.data);
                    break;
            }
        }
        ...
        if (connected) {
            synchronized (this) {
                if (listener != null) {
                    mainLooper.post(() -> {
                        if (listener != null) {
                            listener.onSerialWrite(data);
                        } else {
                            queue1.add(new QueueItem(QueueType.Write, data, null));
                        }
                    });
                } else {
                    queue2.add(new QueueItem(QueueType.Write, data, null));
                }
            }
        }
